...
const Login = () => {
  ...
  const userLogin = async () => {
    ...
    if (res.status === 400 || res.status === 404) {
      ...
      console.log(localStorage.getItem("profileSettingsUpdatedOnce"));
      localStorage.getItem("profileSettingsUpdatedOnce") === "true"
        ? navigate("/shop")
        ...
        <div className="flex flex-col justify-center items-center bg-[#F9F9F9] pr-2 rounded-lg">
          ...
          <div className="form py-4 flex flex-col item-center">
            <input
              type="text"
              placeholder="Name"
              className="w-5/6 sm:w-11/12 h-10 border-2 m-3  border-gray-300 rounded-md p-2"
              value={name}
          ...
          <div className="button">
            <button
              onClick={userLogin}
              className="bg-[#0071FF] text-xl text-white px-8 h-10 rounded-md"
            >
              Login
            </button>
          </div>
