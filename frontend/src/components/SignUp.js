...
const SignUp = () => {
  ...
  const registerUser = async () => {
    ...
    if (data.status === 402) {
      ...
      localStorage.setItem("profileSettingsUpdatedOnce", false);
      navigate("/login");
    }
  };
        ...
        <div className="flex flex-col justify-center items-center bg-[#F9F9F9] pr-2 pb-4 rounded-sm">
          <div className="title text-center">
            <p className="text-2xl text-black py-6">
              {" "}
              SignUp for <b>VIT MarketPlace</b>
            </p>
          </div>
          <div className="form py-4 flex flex-col item-center">
            <input
              type="text"
              placeholder="Name"
              className="w-5/6 md:1 sm:w-11/12 h-10 border-2 m-3  border-gray-300 rounded-md p-2"
              value={name}
          ...
          <div className="button">
            <button
              ...
              className="bg-[#0071FF] text-xl text-white px-8 h-10 rounded-md cursor-pointer"
            >
              Sign Up
            </button>
          </div>
